...
@export var rim_glow_idle: Color = Color(0.37, 0.83, 0.95, 0.3) # Cyan #5FD3F3 30%
@export var rim_glow_active: Color = Color(0.37, 0.83, 0.95, 1.0) # Cyan #5FD3F3 100%
...
func _draw() -> void:
	...
	draw_circle(cap_center, depressed_radius * 0.7, cap_color.lightened(0.1)) # Subtle radial texture
...
func _on_touch_started(finger_id: int, norm_pos: Vector2) -> void:
	...
	if local_pos.distance_to(center) <= BASE_RADIUS:
		...
		Input.vibrate_handheld(10) # Micro-haptic
