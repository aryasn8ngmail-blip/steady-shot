...
@export var base_color: Color = Color("#E5E7EB") # Matte light gray
@export var fill_start_color: Color = Color("#F5A623") # Amber
@export var fill_end_color: Color = Color("#F76B1C")

var is_pressed: bool = false
var current_finger_id: int = -1
var power_level: float = 0.0 # 0.0 to 1.0
...
func _draw() -> void:
	...
	# Base Silhouette with rounded edges
	draw_rect(rect, base_color, true, -1.0)
	draw_rect(rect, Color(0, 0, 0, 0.1), false, 2.0) # Soft inner shadow outline

	# Radial / Vertical Fill Meter
	if power_level > 0.0:
	...
	# Visual depression on press
	if is_pressed:
		draw_rect(rect, Color(0, 0, 0, 0.08), true)

	# Quick pulse on release
	if release_pulse_timer > 0.0:
...
func _on_touch_ended(finger_id: int, _norm_pos: Vector2) -> void:
	if is_pressed and finger_id == current_finger_id:
		...
		current_finger_id = -1
		power_level = 0.0
		release_pulse_timer = 0.2 # 200ms pulse
...
func _update_power(local_pos: Vector2) -> void:
	...
	# Haptic feedback scales with power
